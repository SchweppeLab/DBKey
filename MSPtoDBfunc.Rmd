...
MSPtoDB <- function(Library, FragmentationMode, MassAnalyzer, CollisionEnergy, TMTPro, dbName) {
#reads msp
...
PrecursorMasses <- PrositLib[stri_detect_fixed(PrositLib, "MW:")]
...
blobMass<-if(TMTPro){ 
  return(as_blob(lmap(PeakLists, getMassBlobTMTpro)))}
else {
  return(as_blob(lmap(PeakLists, getMassBlob)))
  }
...
#creates .db
conn3 <- dbConnect(SQLite(), dbname = dbName)
